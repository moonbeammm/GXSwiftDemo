...
extension VDDetailContainerBlocV3 {
    ...
    private func collapseExtraTab() {
        ...
        // 获取配置
        let config = tabContainerBloc.bfc_extraTabViewConfig(in: tabContainerVC)

        // 1. 通知业务方：动画即将开始
        config?.onWillCollapse?(duration)

        // 2. 如果需要进度回调，使用 CADisplayLink
        var displayLink: CADisplayLink?
        if config?.onAnimationProgress != nil {
            let startTime = CACurrentMediaTime()
            displayLink = CADisplayLink(target: self, selector: #selector(updateCollapseProgress))
            displayLink?.add(to: .main, forMode: .common)

            // 保存上下文
            objc_setAssociatedObject(displayLink!, "startTime", startTime, .OBJC_ASSOCIATION_RETAIN)
            objc_setAssociatedObject(displayLink!, "duration", duration, .OBJC_ASSOCIATION_RETAIN)
            objc_setAssociatedObject(displayLink!, "config", config, .OBJC_ASSOCIATION_RETAIN)
        }

        // 3. 执行动画：ExtraTabView向上移动 + Container向上移动
//        UIView.animate(
...
//                    // ✅ 动画完成后，清除ScrollView的offset锁定，允许滚动
//                    self.scrollManager.clearScrollViewOffsets()
//                }
//
//                // 4. 通知业务方：动画完成
//                config?.onDidCollapse?(finished)
//            }
//        )
        UIView.animate(withDuration: duration) {
            ...
            if finished {
                self.extraTabState = .collapsed
                // ✅ 不在这里清除offset锁定！
                // 等待下次手势滑动时，handleUpwardScrolling会检测到shouldIntercept=false，然后清除锁定
                // 这样可以避免动画完成后用户手指还在屏幕上时，ScrollView的offset变成负数的问题
            }
    ...
    /// ✅ 进度回调更新方法
    @objc private func updateCollapseProgress(_ displayLink: CADisplayLink) {
        guard let startTime = objc_getAssociatedObject(displayLink, "startTime") as? CFTimeInterval,
              let duration = objc_getAssociatedObject(displayLink, "duration") as? TimeInterval,
              let config = objc_getAssociatedObject(displayLink, "config") as? BBMPExtraTabConfig else {
            return
        }
        ...
        config.onAnimationProgress?(progress, true)
    ...
    private func expandExtraTab() {
        ...
        // 1. 通知业务方：动画即将开始
        config?.onWillExpand?(duration)

        // 2. 如果需要进度回调，使用 CADisplayLink
        var displayLink: CADisplayLink?
        if config?.onAnimationProgress != nil {
            let startTime = CACurrentMediaTime()
            displayLink = CADisplayLink(target: self, selector: #selector(updateExpandProgress))
            displayLink?.add(to: .main, forMode: .common)
...
//        )
    ...
    /// ✅ 进度回调更新方法
    @objc private func updateExpandProgress(_ displayLink: CADisplayLink) {
        guard let startTime = objc_getAssociatedObject(displayLink, "startTime") as? CFTimeInterval,
              let duration = objc_getAssociatedObject(displayLink, "duration") as? TimeInterval,
              let config = objc_getAssociatedObject(displayLink, "config") as? BBMPExtraTabConfig else {
            return
        ...
        config.onAnimationProgress?(progress, false)
